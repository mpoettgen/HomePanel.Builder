...
If there is no easy way to provide a full integration, the second best thing would be to at least have an option to generate a device configuration file. The remaining steps to compile and upload the firmware can then be done in the ESPHome Builder.
...
### Home Panel Builder UI

The main page of the Home Panel Builder provides the user with a list of the panel designs, that the user has created, each with an option to edit the panel design in the panel designer or to update the panel, if the design has been modified. It also has an option to create a new panel design.

When creating a new panel design the Home Panel Builder asks the user for the name of the panel and the type of the device used to build the panel. With that information, the Home Panel Builder opens the panel designer for the new design.

The main element of the panel designer is the page grid. The user can add tiles to the grid, modify the tiles and drag and drop them around to change tile positions.

The rendering of the page grid should be as close as possible to the rendering of the page on the Home Panel itself, with the exception of editor hints, like grid indicators and edit icons for the editable object.

The UI of the main page is envisioned to be similar to the main page of the ESPHome Device Builder. The UI of the panel designer is envisioned to be similar to the dashboard editor in Home Assistant, but with a fixed grid layout.
